  ...
  return (
    <nav className="flex items-center justufy-between bg-black font-bold text-white h-20 border-b-2 border-black fixed w-full">
      ...
      <div className="ml-auto mr-0 flex items-center pr-10">
        <button type="button" id="order-count" className="px-4 py-2 hover:text-yellow-400 relative" onClick={() => history.push('/basket')}>
          <span className="px-1 bg-white text-black rounded-md absolute text-xs top-0 right-0">
            {list[0].count && list.reduce((acc, rec) => {
        ...
        <button type="button" className="px-4 py-2 md:hidden hover:text-yellow-400" onClick={() => setOpen(!isOpen)}>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
